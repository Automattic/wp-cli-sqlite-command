...
- `[--enable-ast-driver]` Deprecated; accepted as a no-op for backward compatibility.

### `wp sqlite export [<file>]`

Exports the SQLite database to a MySQL-compatible SQL dump file. Generates `DROP TABLE IF EXISTS`, `CREATE TABLE`, and `INSERT INTO` statements for each table. Internal SQLite system tables are never included; the driver's `SHOW TABLES` already excludes them.
...
Lists all user tables in the SQLite database, excluding internal/system tables.
...
- The PDO-based `WP_MySQL_On_SQLite` API (SQLite Database Integration 3.0 or higher) is the only supported driver API
...
- `SQLiteDriverFactory::create_driver()` creates the PDO-based `WP_MySQL_On_SQLite` driver
...
- **Driver API:** Always use `SQLiteDriverFactory::create_driver()`. It returns the PDO-based `WP_MySQL_On_SQLite` driver; SQLite Database Integration 3.0 or higher is required.
